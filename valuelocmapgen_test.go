...
func TestValueSetNewKeyOldTimestampIs0AndNewKeySaved(t *testing.T) {
	tlm := NewValueLocMap(nil).(*valueLocMap)
	keyA := uint64(0)
	keyB := uint64(0)
	timestamp := uint64(2)
	blockID := uint32(1)
	offset := uint32(0)
	length := uint32(0)
	oldTimestamp := tlm.Set(keyA, keyB, timestamp, blockID, offset, length, false)
	if oldTimestamp != 0 {
		t.Fatal(oldTimestamp)
	}
	timestampGet, blockIDGet, offsetGet, lengthGet := tlm.Get(keyA, keyB)
...
func TestValueSetOverwriteKeyOldTimestampIsOldAndOverwriteWins(t *testing.T) {
	...
	offset1 := uint32(0)
	length1 := uint32(0)
	tlm.Set(keyA, keyB, timestamp1, blockID1, offset1, length1, false)
	timestamp2 := timestamp1 + 2
	blockID2 := blockID1 + 1
	offset2 := offset1 + 1
	length2 := length1 + 1
	oldTimestamp := tlm.Set(keyA, keyB, timestamp2, blockID2, offset2, length2, false)
...
func TestValueSetOverwriteKeyOldTimestampIsSameAndOverwriteWins(t *testing.T) {
	...
	offset2 := offset1 + 1
	length2 := length1 + 1
	oldTimestamp := tlm.Set(keyA, keyB, timestamp2, blockID2, offset2, length2, true)
	if oldTimestamp != timestamp1 {
		t.Fatal(oldTimestamp, timestamp1)
	}
	timestampGet, blockIDGet, offsetGet, lengthGet := tlm.Get(keyA, keyB)
...
func TestValueSetOverflowingKeys(t *testing.T) {
	...
	offset1 := uint32(0)
	length1 := uint32(0)
	oldTimestamp := tlm.Set(keyA1, keyB1, timestamp1, blockID1, offset1, length1, false)
	if oldTimestamp != 0 {
		t.Fatal(oldTimestamp)
	}
	timestampGet, blockIDGet, offsetGet, lengthGet := tlm.Get(keyA1, keyB1)
	if timestampGet != timestamp1 {
		t.Fatal(timestampGet, timestamp1)
	}
	if blockIDGet != blockID1 {
	...
	offset2 := offset1 + 1
	length2 := length1 + 1
	oldTimestamp = tlm.Set(keyA2, keyB2, timestamp2, blockID2, offset2, length2, false)
	if oldTimestamp != 0 {
		t.Fatal(oldTimestamp)
	}
	timestampGet, blockIDGet, offsetGet, lengthGet = tlm.Get(keyA2, keyB2)
	if timestampGet != timestamp2 {
		t.Fatal(timestampGet, timestamp2)
	}
	if blockIDGet != blockID2 {
...
func TestValueSetOverflowingKeysReuse(t *testing.T) {
	...
	oldTimestamp = tlm.Set(keyA2, keyB2, timestamp2, uint32(0), offset2, length2, true)
	...
	offset3 := offset1 + 2
	length3 := length1 + 2
	oldTimestamp = tlm.Set(keyA3, keyB3, timestamp3, blockID3, offset3, length3, false)
	if oldTimestamp != 0 {
		t.Fatal(oldTimestamp)
	}
	timestampGet, blockIDGet, offsetGet, lengthGet = tlm.Get(keyA3, keyB3)
...
func TestValueSetOverflowingKeysLots(t *testing.T) {
	...
	for keyB := uint64(0); keyB < 100; keyB++ {
		tlm.Set(keyA, keyB, timestamp, blockID, offset, length, false)
		blockID++
		offset++
		length++
	}
	...
	for keyB := uint64(0); keyB < 75; keyB++ {
		timestampSet := tlm.Set(keyA, keyB, timestamp2, blockID, offset, length, false)
	...
	for keyB := uint64(0); keyB < 50; keyB++ {
		timestampSet := tlm.Set(keyA, keyB, timestamp3, uint32(0), uint32(0), uint32(0), false)
	...
	for keyB := uint64(200); keyB < 300; keyB++ {
		tlm.Set(keyA, keyB, timestamp4, blockID, offset, length, false)
		blockID++
		offset++
		length++
	}
...
func TestValueDiscardMaskNoMatch(t *testing.T) {
	...
	timestamp2, blockID2, offset2, length2 := tlm.Get(keyA, keyB)
...
func TestValueScanCallbackBasic(t *testing.T) {
	...
	offset1 := uint32(2)
	length1 := uint32(3)
	tlm.Set(keyA1, keyB1, timestamp1, blockID1, offset1, length1, false)
	good := false
	stopped, more := tlm.ScanCallback(0, math.MaxUint64, 0, 0, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, timestamp2 uint64, length2 uint32) bool {
...
func TestValueScanCallbackRangeMiss(t *testing.T) {
	...
	tlm.Set(keyA1, keyB1, timestamp1, blockID1, offset1, length1, false)
	good := false
	stopped, more := tlm.ScanCallback(101, math.MaxUint64, 0, 0, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		t.Fatalf("%x %x %d %d\n", keyA2, keyB2, timestamp2, length2)
		return false
	})
	if good {
	...
	good = false
	stopped, more = tlm.ScanCallback(0, 99, 0, 0, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		t.Fatalf("%x %x %d %d\n", keyA2, keyB2, timestamp2, length2)
		return false
	})
	if good {
...
func TestValueScanCallbackMask(t *testing.T) {
	...
	tlm.Set(keyA1, keyB1, timestamp1, blockID1, offset1, length1, false)
	good := false
	stopped, more := tlm.ScanCallback(0, math.MaxUint64, 1, 0, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		if keyA2 == keyA1 && keyB2 == keyB1 {
			if timestamp2 != timestamp1 {
				t.Fatal(timestamp2)
			}
	...
	good = false
	stopped, more = tlm.ScanCallback(0, math.MaxUint64, 2, 0, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		t.Fatalf("%x %x %d %d\n", keyA2, keyB2, timestamp2, length2)
		return false
	})
	if good {
...
func TestValueScanCallbackNotMask(t *testing.T) {
	...
	tlm.Set(keyA1, keyB1, timestamp1, blockID1, offset1, length1, false)
	good := false
	stopped, more := tlm.ScanCallback(0, math.MaxUint64, 0, 2, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		if keyA2 == keyA1 && keyB2 == keyB1 {
			if timestamp2 != timestamp1 {
				t.Fatal(timestamp2)
			}
	...
	good = false
	stopped, more = tlm.ScanCallback(0, math.MaxUint64, 0, 1, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		t.Fatalf("%x %x %d %d\n", keyA2, keyB2, timestamp2, length2)
		return false
	})
	if good {
...
func TestValueScanCallbackCutoff(t *testing.T) {
	...
	tlm.Set(keyA1, keyB1, timestamp1, blockID1, offset1, length1, false)
	good := false
	stopped, more := tlm.ScanCallback(0, math.MaxUint64, 0, 0, 123, 100, func(keyA2 uint64, keyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		if keyA2 == keyA1 && keyB2 == keyB1 {
			if timestamp2 != timestamp1 {
				t.Fatal(timestamp2)
			}
	...
	good = false
	stopped, more = tlm.ScanCallback(0, math.MaxUint64, 0, 0, 122, 100, func(keyA2 uint64, keyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		t.Fatalf("%x %x %d %d\n", keyA2, keyB2, timestamp2, length2)
		return false
	})
	if good {
...
func TestValueScanCallbackMax(t *testing.T) {
	...
	for i := 0; i < 4000; i++ {
		...
		tlm.Set(keyA, 0, 1, 2, 3, 4, false)
	}
	count := 0
	stopped, more := tlm.ScanCallback(0, math.MaxUint64, 0, 0, math.MaxUint64, 50, func(keyA2 uint64, keyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		count++
		return true
	})
	if count != 50 {
	...
	count = 0
	stopped, more = tlm.ScanCallback(0, math.MaxUint64, 0, 0, math.MaxUint64, 5000, func(keyA2 uint64, keyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		count++
		return true
	})
	if count != 4000 {
...
func TestValueExerciseSplitMergeDiscard(t *testing.T) {
	...
	kt := func(ka uint64, kb uint64, ts uint64, b uint32, o uint32, l uint32) {
		tlm.Set(ka, kb, ts, b, o, l, false)
		ts2, b2, o2, l2 := tlm.Get(ka, kb)
